...
const colorTheme = ['#5BC0EB', '#FDE74C', '#9BC53D', '#E55934', '#FA7921', '#fa3081'];
...
function renderChart(angleKey: string, labelKey?: string, radiusKey?: string) {
  ...
  canvas.element.addEventListener('mouseup', (_e: any) => {
    isDragging = false;
  });
  ...
  function draw() {
    ...
    angleDataRatios.reduce((start, end) => {
      ...
      calloutData.push({
        start: [centerX + midCos * radius, centerY + midSin * radius],
        end: [centerX + midCos * (radius + calloutLength), centerY + midSin * (radius + calloutLength)],
        label:
          span > minLabelAngle
            ? [
