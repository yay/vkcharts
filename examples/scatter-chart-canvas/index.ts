...
import carData, { type CarInfo } from './auto-mpg';

const colors = ['#5BC0EB', '#FDE74C', '#9BC53D', '#E55934', '#FA7921', '#fa3081'];
...
const data = [generateData(names), generateData(names), generateData(names)];
...
function renderChart(data: any[][], configs: SeriesConfig[], minRadius = 2, maxRadius = 16) {
  ...
  const radiusData = configs.map((config, i) =>
    data[i].map((datum) => (config.radiusKey ? datum[config.radiusKey] : defaultRadius) as number)
  );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  {
    const carsByCountry: { [key: string]: CarInfo[] } = {};
    carData.forEach((car) => {
