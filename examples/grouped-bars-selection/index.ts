...
function renderChart() {
  const colors = ['#53AFD6', '#FDE95C', '#A4CA4E', '#E76846', '#FA8535'];
  ...
  Selection.select(barGroup)
    .selectAll()
    .setData(data, (_node, datum) => datum.category)
    .enter.append(Group)
    .each((group, datum) => {
      group.translationX = xGroupScale.convert(datum.category);
    })
    .selectAll()
    .setData((_parent, _datum, i) => {
      return yData[i];
    })
    .enter.call((enter) =>
      enter.append(Rect).each((rect, datum, i) => {
    ...
    .call((enter) =>
      enter.append(Text).each((label, datum, i) => {
        label.text = yNames[i];
        label.textAlign = 'center';
