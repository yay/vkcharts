...
import { VkChart, type VkChartOptions, Caption, LinearScale } from '../../lib/main';
import { HierarchyChart } from '../../lib/chart/hierarchyChart';
import { TreemapSeries } from '../../lib/chart/series/hierarchy/treemapSeries';
import { hierarchy } from '../../lib/layout/hierarchy';
import { Treemap } from '../../lib/layout/treemap';
...
function createOrgTreeMap() {
  ...
  function update() {
    ...
    const dataRoot = hierarchy(data).sum((datum) => (datum.children ? 0 : 1));
    const treemapLayout = new Treemap();
    treemapLayout.size = [width, height];
    treemapLayout.paddingTop = (node) => {
    ...
    const nodePadding = () => 4;
    treemapLayout.paddingRight = nodePadding;
    treemapLayout.paddingBottom = nodePadding;
    treemapLayout.paddingLeft = nodePadding;
    const descendants = treemapLayout.processData(dataRoot).descendants();
    ...
    groupSelection.selectByClass(Rect).each((rect, datum, _index) => {
    ...
    groupSelection.selectByClass(Text).each((text, datum) => {
      ...
      if (bbox) {
        text.visible =
          (!isRoot && datum.x1 - datum.x0 - 2 * 2 > bbox.width && datum.y1 - datum.y0 - 2 * 2 > bbox.height) ||
          datum.hasTitle;
      }
      text.x = datum.x0 + 3;
      text.y = datum.y0 + 4;
...
function createStockTreeMapChart() {
  ...
  function setupStockTreemap() {
    ...
    series.tooltip.renderer = (params) => {
      ...
      //     ? datum.parent.depth
      //         ? datum.parent.data[params.labelKey]
      //         : customRootText
      //     : customRootText;
      let content = '<div>';
      let ellipsis = false;
...
function createStockTreeMapChartViaFactory() {
  ...
  VkChart.create({
    type: 'hierarchy',
    // theme: 'vk-default-dark',
    theme: {
      baseTheme: 'vk-default-dark',
      overrides: {
        hierarchy: {
          series: {
            treemap: {
