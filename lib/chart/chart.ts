...
.vk-chart-tooltip-hidden {
    top: -10000px !important;
}

.vk-chart-tooltip-title {
    font-weight: bold;
    padding: 7px;
...
.vk-chart-tooltip-content:empty {
    padding: 0;
    height: 7px;
}

.vk-chart-tooltip-arrow::before {
    content: "";
...
export abstract class Chart extends Observable {
  ...
  static readonly defaultTooltipClass = 'vk-chart-tooltip';
  ...
  protected handleTooltip(event: MouseEvent) {
    const {
      lastPick,
      tooltip: { tracking: tooltipTracking },
    } = this;
    ...
    if (pick && pick.node instanceof Shape) {
      ...
      if (!node.datum.point || !tooltipTracking) {
        if (
          !lastPick || // cursor moved from empty space to a node
          lastPick.node !== node
        ) {
