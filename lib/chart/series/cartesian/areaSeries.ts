...
export class AreaSeries extends CartesianSeries {
  ...
  private updateFillSelection(): void {
    ...
    const enterFills = updateFills.enter.append(Path).each((path) => {
      path.lineJoin = 'round';
  ...
  private updateStrokeSelection(): void {
    ...
    const enterStrokes = updateStrokes.enter.append(Path).each((path) => {
      path.fill = undefined;
  ...
  private updateMarkerNodes(): void {
    ...
    this.markerSelection.each((node, datum) => {
      ...
      const stroke =
        isDatumHighlighted && highlightedStroke !== undefined ? highlightedStroke : marker.stroke || datum.stroke;
      const strokeWidth =
