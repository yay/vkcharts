  ...
  update() {
    ...
    if (this.gridLength && gridStyle.length) {
      ...
      if (this.radialGrid) {
        ...
        gridLines = groupSelection.selectByTag<Arc>(Tags.GridLine).each((arc, datum) => {
        ...
        gridLines = groupSelection.selectByTag<Line>(Tags.GridLine).each((line) => {
          line.x1 = 0;
    ...
    const labelSelection = groupSelection.selectByClass(Text).each((node, datum, index) => {
