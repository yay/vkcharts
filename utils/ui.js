export function createButton(text, action, parent = document.body) {
...
const wrapInLabel = (labelText, ele) => {
    const label = document.createElement('label');
    label.appendChild(document.createTextNode(labelText));
    label.appendChild(ele);
    return label;
};
export function createDropdown(labelText, values, action) {
    ...
    document.body.appendChild(wrapInLabel(labelText, select));
}
export function createSliderValues(start, range, steps) {
...
export function createSlider(text, values, action, parent = document.body) {
...
export function createRangeSlider(text, range, step, action) {
    const id = String(Date.now());
    const sliderId = 'slider-' + id;
    const wrapper = document.createElement('div');
    // wrapper.style.display = 'inline-flex';
...
export function createSlider2(options = {}) {
