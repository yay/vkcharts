...
export const lightThemes: ThemeMap = {
    ...
    'null': lightTheme,
    'vk-default': lightTheme,
    'vk-material': new MaterialLight(),
    'vk-pastel': new PastelLight(),
    'vk-solar': new SolarLight(),
    'vk-vivid': new VividLight(),
};

export const darkThemes: ThemeMap = {
    'undefined': darkTheme,
    'null': darkTheme,
    'vk-default-dark': darkTheme,
    'vk-material-dark': new MaterialDark(),
    'vk-pastel-dark': new PastelDark(),
    'vk-solar-dark': new SolarDark(),
    'vk-vivid-dark': new VividDark(),
};
...
export function getChartTheme(value?: string | ChartTheme | VkChartTheme): ChartTheme {
    if (value instanceof ChartTheme) {
        return value;
    }

    const stockTheme = themes[value as VkChartThemeName];

    if (stockTheme) {
        return stockTheme;
    }

    value = value as VkChartTheme;
...
type VkChartType<T> =
    T extends VkCartesianChartOptions ? CartesianChart :
    T extends VkPolarChartOptions ? PolarChart :
    T extends VkHierarchyChartOptions ? HierarchyChart :
    never;

let firstColorIndex = 0;

export abstract class VkChart {
    static create<T extends VkChartOptions>(options: T, container?: HTMLElement, data?: any[]): VkChartType<T> {
    ...
    static update<T extends VkChartOptions>(chart: VkChartType<T>, options: T, container?: HTMLElement, data?: any[]) {
        if (!(chart && options)) {
            return;
        }
